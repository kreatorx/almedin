...
function resetDrawingState() { isDrawing = false; dimStep = 0; dimP1 = null; dimP2 = null; isBoxSelecting = false; if(activeGrip) cancelGripMove(); hideDynamicInput(); hidePropsMenu(); }
...
function drawEverything() {
    ...
    if (paperDim) {
        ...
        if (isPrinting) {
            ctx.save();
            ctx.translate(paperDim.w / 2, paperDim.margin / 2);
            ctx.scale(1, -1);
...
function openPropsMenu(x, y) {
    ...
    let line = selectedElements[0];
    
    document.getElementById('prop-thickness').value = line.thickness || 0.2;
    document.getElementById('thick-val').innerText = (line.thickness || 0.2).toFixed(2);
    document.getElementById('prop-dash-len').value = line.dashLength || 10;
    document.getElementById('prop-dash-gap').value = line.dashGap || 5;
    document.getElementById('prop-hex-color').value = line.color || '#ffffff';
    
    updateLineTypeButtons(line.lineType || 'solid');
    
    let menuW = 250, menuH = 340;
...
function updateLineFromMenu() {
    ...
    line.dashLength = parseFloat(document.getElementById('prop-dash-len').value) || 10;
    line.dashGap = parseFloat(document.getElementById('prop-dash-gap').value) || 5;
...
function drawAutoCADDimension(p1, p2, offset, isPreview = false, isSelected = false, dimType = 'aligned') {
    ...
    if (dimType === 'horizontal') {
        ...
        dimLineP1 = { x: p1.x, y: dimLineY }; dimLineP2 = { x: p2.x, y: dimLineY };
        angle = 0;
        ...
        dimLineP1 = { x: dimLineX, y: p1.y }; dimLineP2 = { x: dimLineX, y: p2.y };
        angle = Math.PI / 2;
        ctx.beginPath(); ctx.moveTo(p1.x, p1.y); ctx.lineTo(dimLineP1.x, dimLineP1.y); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(p2.x, p2.y); ctx.lineTo(dimLineP2.x, dimLineP2.y); ctx.stroke();
    } 
...
canvas.addEventListener('mousedown', (e) => {
    ...
    if (e.button === 2) { 
        ...
        if (activeGrip) {
            cancelGripMove();
        } else if (mode === 'select' && selectedElements.length === 1 && selectedElements[0].type === 'line') {
    ...
    if (mode === 'select') {
        ...
        if (!isDrawing) { 
            ...
            if (startPoint.x !== pos.x || startPoint.y !== pos.y) { 
                elements.push({ 
                    type: 'line', p1: { ...startPoint }, p2: { ...pos },
    ...
    if (e.key === 'Enter') {
        ...
        if (val.includes(',')) {
            ...
            if (mode === 'line' && isDrawing) { 
                ...
                startPoint = { x: finalWorldX, y: finalWorldY }; currentPoint = { ...startPoint }; 
            } 
            else if (mode === 'select' && activeGrip && selectedElements[0]) {
            ...
            if (mode === 'line' && isDrawing) {
                ...
                startPoint = finalPoint; currentPoint = finalPoint;
